...
describe("Test update user master", () =>{

    it("Este test actualiza el usuario master", () =>{
        ...
        cy.log("Acutalización de usuario master")
        cy.xpath("//div[@class='db-option-name ng-star-inserted'][contains(.,'4. Códigos de Usuarios')]").should("be.visible").click()
        cy.xpath("//div[@class='flex-grow-1 exp-section-title'][contains(.,'1 - user1')]").should("be.visible").click()
        cy.xpath("//span[@class='mat-button-wrapper'][contains(.,'Editar')]").should("be.visible").click()
        //cy.wait(2000)
        cy.xpath("(//span[@class='mat-button-wrapper'][contains(.,'Guardar')])[2]").should("be.visible").click()

        cy.log("Validar que se guardo la actualización del usuario")
        cy.xpath("//div[@class='m-mat-snackbar__message'][contains(.,'Se actualizó correctamente el usuario:user1')]").should("have.text","Se actualizó correctamente el usuario:user1")
